...
@section('content')
    ...
    <section class="relative overflow-hidden rounded-2xl bg-brand-600 px-6 py-8 text-white shadow-sm">
        <div class="relative z-10">
            <p class="text-xs font-semibold uppercase tracking-widest text-brand-100">Selamat datang</p>
            <h1 class="mt-1 text-2xl font-bold sm:text-3xl">Silakan pilih menu favorit Anda</h1>
            <p class="mt-2 text-sm text-brand-100">Pesan dari meja, disiapkan dapur, dan diantar saat sudah siap.</p>
        </div>
        <svg xmlns="http://www.w3.org/2000/svg" class="pointer-events-none absolute -bottom-6 -right-6 h-40 w-40 text-white/10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v10m0 0a4 4 0 100 8c2.21 0 4-1.79 4-4M12 14c.5 1 1.5 2.5 3 2.5a2.5 2.5 0 100-5c-1.5 0-2.5 1-3 2.5z"/></svg>
    </section>

    <nav aria-label="Kategori menu" class="mt-6 -mx-4 overflow-x-auto px-4 pb-1 lg:mx-0 lg:px-0">
        <div class="flex gap-2">
            <a href="{{ route('menu.index') }}"
               class="{{ ! isset($category) ? 'bg-brand-600 text-white' : 'border border-gray-200 bg-white text-gray-700 hover:bg-gray-50' }} shrink-0 rounded-full px-4 py-2 text-sm font-medium transition">
                Semua Menu
            </a>
            @foreach ($categories as $cat)
                <a href="{{ route('menu.category', $cat) }}"
                   class="{{ $category?->slug === $cat->slug ? 'bg-brand-600 text-white' : 'border border-gray-200 bg-white text-gray-700 hover:bg-gray-50' }} shrink-0 rounded-full px-4 py-2 text-sm font-medium transition">
                    {{ $cat->name }}
                    <span class="ml-1 {{ $category?->slug === $cat->slug ? 'text-brand-100' : 'text-gray-400' }}">{{ $cat->active_products_count }}</span>
                </a>
            @endforeach
        </div>
    </nav>
    ...
    <div class="mt-6">
        ...
        @if (! $products->isEmpty() && request('q'))
            <p class="mb-3 text-xs text-gray-500">Hasil pencarian untuk "{{ request('q') }}".</p>
        @endif
        ...
        @if ($products->isEmpty())
            <div class="card p-10 text-center">
                ...
                @if (request('q'))
                    <a href="{{ route('menu.index') }}" class="mt-3 inline-block text-sm font-medium text-brand-600 hover:text-brand-700">Hapus pencarian</a>
                @endif
            </div>
        @else
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
                @foreach ($products as $product)
                    <article class="card group overflow-hidden transition duration-200 hover:-translate-y-0.5 hover:shadow-md">
                        <div class="relative aspect-[4/3] overflow-hidden bg-gray-100">
                            ...
                            @if ($product->isSoldOut())
                                <div class="absolute inset-0 flex items-center justify-center bg-gray-900/50">
                                    <span class="rounded-full bg-white px-4 py-1.5 text-xs font-bold uppercase tracking-wide text-gray-900">Habis</span>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="p-4">
                            ...
                            <div class="mt-4 flex items-center justify-between gap-2">
                                ...
                                @elseif ($qty > 0)
                                    <form method="POST" action="{{ route('cart.update', $product->id) }}" class="flex items-center rounded-lg border border-gray-200 bg-white">
                                        @csrf
                                        <button type="submit" name="quantity" value="{{ $qty - 1 }}" aria-label="Kurangi {{ $product->name }}" class="px-3 py-1.5 text-sm font-semibold text-gray-600 hover:text-brand-600">−</button>
                                        <span class="min-w-7 text-center text-sm font-semibold text-gray-900" aria-label="{{ $qty }} di keranjang">{{ $qty }}</span>
                                        <button type="submit" name="quantity" value="{{ $qty + 1 }}"
                                                @if ($product->isLimitedStock() && $qty >= $product->stock) disabled @endif
                                                aria-label="Tambah {{ $product->name }}" class="px-3 py-1.5 text-sm font-semibold text-gray-600 hover:text-brand-600 disabled:opacity-40">+</button>
                                    </form>
                                @else
