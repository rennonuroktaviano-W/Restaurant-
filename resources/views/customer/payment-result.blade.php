        ...
        <div class="card overflow-hidden text-center">
            <div class="px-6 py-10">
                @if ($payment->status === 'paid')
                    ...
                    <p class="mt-2 text-sm text-gray-500">Order <strong>{{ $payment->order->order_number }}</strong> sudah lunas dan diproses.</p>
                @elseif ($payment->status === 'failed')
                    ...
                    <p class="mt-2 text-sm text-gray-500">Silakan ulangi pembayaran untuk order {{ $payment->order->order_number }}.</p>
                @else
                    ...
                    <h1 class="text-xl font-bold text-gray-900">Pembayaran Kadaluarsa / Menunggu</h1>
                    <p class="mt-2 text-sm text-gray-500">Status: {{ $payment->status }}</p>
                @endif
