...
@section('title', 'Tambah Area - ' . config('app.name'))
@section('header', 'Tambah Area')

@section('content')
    <div class="mb-5 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Area</h1>
        <a href="{{ route('admin.areas.index') }}" class="btn btn-secondary">Kembali</a>
    </div>

    <form method="POST" action="{{ route('admin.areas.store') }}" class="card p-6">
        @csrf
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <select name="type" id="type" class="select @error('type') border-red-400 @enderror" required>
                    <option value="restaurant" @selected(old('type') === 'restaurant')>Restaurant</option>
                    <option value="pool" @selected(old('type') === 'pool')>Pool</option>
