    ...
    <form method="POST" action="{{ route('admin.payment-methods.update', $method) }}" class="card mx-auto max-w-2xl p-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <input id="name" type="text" name="name" value="{{ old('name', $method->name) }}" required class="input @error('name') border-red-400 @enderror">
                @error('name')
            ...
            <div>
                ...
                <input id="code" type="text" name="code" value="{{ old('code', $method->code) }}" required class="input @error('code') border-red-400 @enderror">
                @error('code')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input id="sort_order" type="number" name="sort_order" value="{{ old('sort_order', $method->sort_order) }}" min="0" max="9999" class="input @error('sort_order') border-red-400 @enderror">
                @error('sort_order')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-4">
            ...
            <textarea id="config" name="config" rows="4" class="input font-mono @error('config') border-red-400 @enderror">{{ old('config', json_encode($method->config, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)) }}</textarea>
            @error('config')
                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
            @enderror
