...
@section('title', 'Tambah Meja - ' . config('app.name'))
@section('header', 'Tambah Meja')

@section('content')
    <div class="mb-5 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Meja</h1>
        <a href="{{ route('admin.dining-tables.index') }}" class="btn btn-secondary">Kembali</a>
    </div>

    <form method="POST" action="{{ route('admin.dining-tables.store') }}" class="card p-6">
        @csrf
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="table_number" id="table_number" value="{{ old('table_number') }}" class="input @error('table_number') border-red-400 @enderror" required>
                @error('table_number')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
