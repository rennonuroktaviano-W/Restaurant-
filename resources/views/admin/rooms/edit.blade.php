...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.rooms.update', $room) }}" class="card p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <select name="area_id" id="area_id" class="select @error('area_id') border-red-400 @enderror" required>
                    ...
                    @foreach ($areas as $area)
                        <option value="{{ $area->id }}" @selected(old('area_id', $room->area_id) == $area->id)>{{ $area->name }}</option>
                    @endforeach
                </select>
                @error('area_id')
            ...
            <div>
                ...
                <input type="text" name="room_number" id="room_number" value="{{ old('room_number', $room->room_number) }}" class="input @error('room_number') border-red-400 @enderror" required>
                @error('room_number')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $room->name) }}" class="input @error('name') border-red-400 @enderror">
                @error('name')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" class="select @error('status') border-red-400 @enderror" required>
                    ...
                    <option value="reserved" @selected(old('status', $room->status) === 'reserved')>Dipesan</option>
                </select>
                @error('status')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="sm:col-span-2">
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $room->is_active)) class="rounded border-gray-300 text-brand-600 focus:ring-brand-500">
