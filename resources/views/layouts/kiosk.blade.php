...
<body class="flex min-h-screen flex-col bg-cream-100 font-sans text-ink-800 antialiased">
    ...
    <header x-data="{ menuOpen: false }" class="fixed inset-x-0 top-0 z-40">
        <nav aria-label="{{ __('nav.main_navigation') }}"
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2.5" aria-label="{{ $siteName }} — {{ __('nav.home') }}">
            ...
            <div class="hidden items-center gap-8 lg:flex">
                <a href="{{ route('menu.index') }}" class="text-sm font-medium text-ink-700 transition hover:text-forest-700">{{ __('nav.menu') }}</a>
                @if ($landingNav)
                    <a href="#signature" class="text-sm font-medium text-ink-700 transition hover:text-forest-700">{{ __('home.featured') }}</a>
                    <a href="#story" class="text-sm font-medium text-ink-700 transition hover:text-forest-700">{{ __('home.about') }}</a>
                    <a href="#story" class="text-sm font-medium text-ink-700 transition hover:text-forest-700">{{ __('home.experience') }}</a>
                    <a href="#location" class="text-sm font-medium text-ink-700 transition hover:text-forest-700">{{ __('home.location') }}</a>
                @endif
            </div>
        ...
        <div id="mobile-menu" x-show="menuOpen" x-cloak x-transition
            ...
            <div class="mx-auto max-w-[1280px] space-y-1 px-5 py-4 sm:px-8">
                <a href="{{ route('menu.index') }}" @click="menuOpen = false" class="block rounded-lg px-3 py-2.5 text-sm font-medium text-ink-800 hover:bg-cream-200/60">{{ __('nav.menu') }}</a>
                @if ($landingNav)
                    <a href="#signature" @click="menuOpen = false" class="block rounded-lg px-3 py-2.5 text-sm font-medium text-ink-800 hover:bg-cream-200/60">{{ __('home.featured') }}</a>
                    <a href="#story" @click="menuOpen = false" class="block rounded-lg px-3 py-2.5 text-sm font-medium text-ink-800 hover:bg-cream-200/60">{{ __('home.about') }}</a>
                    <a href="#story" @click="menuOpen = false" class="block rounded-lg px-3 py-2.5 text-sm font-medium text-ink-800 hover:bg-cream-200/60">{{ __('home.experience') }}</a>
                @endif
            </div>
        </div>
    </header>
    ...
    <footer class="border-t border-ink-900/10 bg-forest-900 text-cream-200">
        <div class="mx-auto grid max-w-[1280px] gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[1.4fr_1fr_1.2fr]">
            ...
            <nav aria-label="{{ __('nav.menu_navigation') }}">
                <h2 class="text-sm font-semibold tracking-wide text-cream-50 uppercase">{{ __('nav.explore') }}</h2>
                <ul class="mt-4 space-y-2.5 text-sm">
                    <li><a href="{{ route('menu.index') }}" class="text-cream-300/80 transition hover:text-cream-50">{{ __('nav.view_menu') }}</a></li>
                    <li><a href="{{ route('menu.index') }}" class="text-cream-300/80 transition hover:text-cream-50">{{ __('nav.order_now') }}</a></li>
                    <li><a href="{{ route('cart.index') }}" class="text-cream-300/80 transition hover:text-cream-50">{{ __('nav.cart') }}</a></li>
                </ul>
            </nav>

            <div>
                <h2 class="text-sm font-semibold tracking-wide text-cream-50 uppercase">{{ __('home.opening_hours') }}</h2>
                <p class="mt-4 text-sm text-cream-300/80">{{ __('home.daily') }}</p>
                <p class="mt-1 text-sm text-cream-300/80">{{ __('home.hours_value') }}</p>
                @php
        ...
        <div class="border-t border-cream-50/10">
            <div class="mx-auto flex max-w-[1280px] flex-col items-center justify-between gap-2 px-5 py-6 text-center sm:flex-row sm:px-8 sm:text-left">
                <p class="text-sm text-cream-300/70">&copy; {{ date('Y') }} {{ $siteName }}. {{ __('common.all_rights_reserved') }}</p>
                <p class="text-xs text-cream-300/50">{{ __('home.menu_subject_to_change') }}</p>
            </div>
        </div>
    </footer>
